...
async function resolveModelList(requested, category) {
  ...
  const available = allModels.filter(m => {
    if (EXCLUDED_MODELS.has(m)) return false;
    if (QUARANTINED_MODELS.has(m) && !process.env.INCLUDE_QUARANTINED) return false;
    return true;
  });
  const seen = new Set();
  const list = [];
  ...
  // 1. User override from model-map.json (non-empty value)
  if (userMap[requested]) push(userMap[requested]);

  // 2. Category-aware ordering
  if (SMALL_FIRST_CATEGORIES.has(category)) {
    // Fast/cheap tasks: smallest first; on size tie, non-thinking before thinking
    const sorted = [...available].sort((a, b) => {
      const sd = _estimateSize(a) - _estimateSize(b);
      if (sd !== 0) return sd;
      return (_isThinking(a) ? 1 : 0) - (_isThinking(b) ? 1 : 0);
    });
    for (const m of sorted) push(m);
  } else if (LARGE_FIRST_CATEGORIES.has(category)) {
    // Large-context tasks: biggest first; on size tie, thinking before non-thinking
    const sorted = [...available].sort((a, b) => {
      const sd = _estimateSize(b) - _estimateSize(a);
      if (sd !== 0) return sd;
      return (_isThinking(b) ? 1 : 0) - (_isThinking(a) ? 1 : 0);
    });
    for (const m of sorted) push(m);
  } else {
    // Default: exact match, then heuristic score descending
  ...
  // 3. Passthrough fallback (last resort, may be rejected by LM Studio)
