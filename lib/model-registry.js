...
async function resolveModelList(requested, category) {
  ...
  if (SMALL_FIRST_CATEGORIES.has(category)) {
    // Fast/cheap tasks: prefer smallest model first
    const sorted = [...available].sort((a, b) => _estimateSize(a) - _estimateSize(b));
    for (const m of sorted) push(m);
  } else if (LARGE_FIRST_CATEGORIES.has(category)) {
    // Large-context tasks: prefer biggest model first
    const sorted = [...available].sort((a, b) => _estimateSize(b) - _estimateSize(a));
    for (const m of sorted) push(m);
  } else {
