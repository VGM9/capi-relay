...
/**
 * Try one model using streaming, collecting content tokens until we have a
 * non-empty result, then destroy the connection. This avoids waiting for the
 * full thinking block on Qwen3/reasoning models — we get the answer the moment
 * the model emits actual content and cancel immediately.
 *
      ...
      upstream.on('data', chunk => {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (delta) {
              content += delta;
              // Once we have non-whitespace content, we have what we need.
              // Destroy the connection — llama.cpp stops generating immediately.
...
async function forwardToLocal(body, res, category = 'unknown') {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    const t0     = Date.now();
    // For models with thinking enabled ('-thinking' suffix): append /no_think
    // to the last user message as a fallback suppression hint.
    // The base qwen/qwen3.5-9b has thinking disabled at the LM Studio model
    // config level, so no suffix needed there.
    let msgs = messages;
    if (/-thinking/i.test(model)) {
      msgs = _appendNoThink(messages);
    }

    // title/progress: use streaming + early exit so we don't wait for the full
    // thinking block — we cancel the moment the first content token arrives.
    let result;
