...
async function forwardToLocal(body, res, category = 'unknown') {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    const t0     = Date.now();
    // For short-output tasks, disable chain-of-thought on Qwen3 thinking models so
    // they don't burn all of max_tokens on <think> and return empty content.
    const modelOverrides = { ...overrides };
    if ((category === 'title' || category === 'progress') && /qwen.*3|qwen3/i.test(model)) {
