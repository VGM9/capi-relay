...
async function forwardToLocal(body, res, category = 'unknown') {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    const t0     = Date.now();
    // For Qwen3 thinking models: append /no_think to the last user message.
    // This is the llama.cpp chat-template token — enable_thinking:false is
    // silently ignored by LM Studio's backend.
    let msgs = messages;
