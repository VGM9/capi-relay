...
const { resolveModelList, noteModelAttemptStart, noteModelAttemptEnd } = require('./model-registry');
...
const CATEGORY_PARAMS = {
  title:      { repeat_penalty: 1.30, frequency_penalty: 0.1, presence_penalty: 0.0, temperature: 0.05, max_tokens: 4096, timeoutMs: 15_000 },
  progress:   { repeat_penalty: 1.30, frequency_penalty: 0.1, presence_penalty: 0.0, temperature: 0.05, max_tokens: 4096, timeoutMs: 20_000 },
  compaction: { repeat_penalty: 1.10, frequency_penalty: 0.0, presence_penalty: 0.0, max_tokens: 4096, timeoutMs: 60_000 },
  default:    { repeat_penalty: 1.10, frequency_penalty: 0.0, presence_penalty: 0.0, max_tokens: 4096, timeoutMs: 30_000 },
};
 ...
 */
function _tryModelStream(model, body, overrides = {}, timeoutMs = MODEL_TIMEOUT) {
  return new Promise((resolve) => {
    ...
    req.setTimeout(timeoutMs, () => { req.destroy(); done(false, `timeout after ${timeoutMs}ms`); });
 ...
 */
function _proxyStream(model, body, overrides, res, timeoutMs = MODEL_TIMEOUT) {
  return new Promise((resolve) => {
    const localBody = JSON.stringify({
      model,
      messages: body.messages,
    ...
    req.setTimeout(timeoutMs, () => { req.destroy(); fail(`timeout after ${timeoutMs}ms`); });
 ...
 */
function _tryModel(model, body, overrides = {}, timeoutMs = MODEL_TIMEOUT) {
  return new Promise((resolve) => {
    const localBody = JSON.stringify({
      model,
      messages: body.messages,
...
async function forwardToLocal(body, res, category = 'unknown') {
  ...
  const timeoutMs = catParams.timeoutMs || MODEL_TIMEOUT;
  const overrides = {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    if (category === 'title' || category === 'progress') {
      ...
      const r = await _tryModelStream(model, { ...body, messages: msgs }, overrides, timeoutMs);
      if (r.ok) {
        result = {
          ok: true,
          data: JSON.stringify({
      ...
      result = await _proxyStream(model, { ...body, messages: msgs }, overrides, res, timeoutMs);
    } else {
      result = await _tryModel(model, { ...body, messages: msgs }, overrides, timeoutMs);
    }
    const ms     = Date.now() - t0;
    noteModelAttemptEnd(model, { ok: !!result.ok, latencyMs: ms, timeout: !result.ok && /timeout/.test(result.reason || '') });
